...
SELECT
  ...
  users.first_name,
  users.last_name
FROM posts AS p JOIN users ON (p.poster_id = users.id)
WHERE users.id = 200;
--p.*
--indent
--sep steps
...
FROM posts 
JOIN users ON (posts.poster_id = users.id) 
JOIN comments ON (comments.commenter_id = users.id)
WHERE comments.body ILIKE '%matrix%';
...
SELECT COUNT (*)
FROM comments 
JOIN posts ON (comments.post_id = posts.poster_id)
